...
local function CanUseSpell(spellName)
    ...
    -- spell.LevelRequired needs a +1 shift to compensate for the array's
    -- 1-indexed job data structure: e.g. Ionohelix II is SCH-main-only via
    -- 1200 JP, and SCH's job id (20) + 1 = 21 is exactly where 1200 shows up
    -- in the raw array -- confirmed against real game rules, not just the
    -- data shape. (A GEO main character will correctly read -1 here and be
    -- blocked outright, since GEO has no path to Helix II at all -- only
    -- Helix *I* via SCH subjob, which is a separate spell resource with its
    -- own real level requirement rather than a JP master-level unlock.)
    -- Checked separately per job: some spells (e.g. Stun) are only learnable
    -- by a job when it's your SUBJOB (e.g. DRK sub on RDM), and each job has
    -- its own LevelRequired entry -- checking only the main job's index
    -- against both main and sub level would wrongly reject those spells
    -- whenever the main job itself can't learn them.
    local mainLevelRequired = spell.LevelRequired[mainJobId + 1];
    local subLevelRequired  = (subJobId ~= 0) and spell.LevelRequired[subJobId + 1] or nil;

    DebugMessage(('LevelRequired for %s -- main %s(%d): %s, sub %s(%d): %s (mainLv=%d, subLv=%d)')
        :format(spellName, tostring(JobIDs[mainJobId]), mainJobId, tostring(mainLevelRequired),
                tostring(JobIDs[subJobId]), subJobId, tostring(subLevelRequired),
                player:GetMainJobLevel(), player:GetSubJobLevel()));

    -- One-time diagnostic: if BOTH lookups come back unusable, dump the
    -- entire LevelRequired array (job ids 0-23) for this spell so we can see
    -- whether the data landed at a different index than expected.
    -- Prints unconditionally (not gated by DebugMode) since it only fires once.
    --
    -- Labeled per the real +1 shift convention used above: raw array index i
    -- holds job id (i - 1)'s value (so index 0 is unused/padding, index 1 is
    -- job 0/NON, ..., index 21 is job 20/SCH, etc.) -- NOT job id i. Mislabeling
    -- this previously led to misreading a SCH-only JP value as if it were GEO's.
    local mainUnusable = (mainLevelRequired == nil or mainLevelRequired == -1);
    local subUnusable  = (subLevelRequired == nil or subLevelRequired == -1);
    if (mainUnusable and subUnusable and not levelrequired_dump_shown) then
        levelrequired_dump_shown = true;
        local parts = {};
        for i = 0, 23 do
            local ok, val = pcall(function () return spell.LevelRequired[i]; end);
            local jobLabel = (i == 0) and 'pad' or tostring(JobIDs[i - 1]);
            parts[#parts + 1] = ('%d:%s=%s'):format(i, jobLabel, ok and tostring(val) or 'ERR');
        end
    ...
    if (not mainUnusable) then
        if (mainLevelRequired >= 100) then
            local jpSpent = player:GetJobPointsSpent(mainJobId) or 0;
            local passed  = jpSpent >= mainLevelRequired;
            DebugMessage(('JP check for %s (%s): spent %d, need %d -- %s')
                :format(spellName, JobIDs[mainJobId], jpSpent, mainLevelRequired, passed and 'PASSED' or 'FAILED'));
            if (passed) then return true; end
        elseif (mainLevelRequired <= player:GetMainJobLevel()) then
            return true;
        end
    end

    if (not subUnusable and subLevelRequired <= player:GetSubJobLevel()) then
        return true;
    end
...
local function AttemptElementalBurstTier(chain, startIndex, burstsSoFar)
    ...
    local SPELL_MAPPING_OVERRIDES = {
        ...
        ['Kaustra'] = { ['I'] = 'Kaustra' }, 
        ['Impact']  = { ['I'] = 'Impact' },
        -- Earth-elemental, single-tier (no numbered variant). Deals no direct
        -- damage -- magic bursting it only boosts magic accuracy (better
        -- Petrify landing rate), not damage. Set situationally in
        -- AutoBurst_config.lua's BurstMagic table for Umbra/Darkness/
        -- Gravitation/Scission when the extra accuracy is worth it, same as
        -- Death/Impact above.
        ['Break']   = { ['I'] = 'Break' },
    }
