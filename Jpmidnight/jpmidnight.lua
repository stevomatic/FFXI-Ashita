...
-- Confirmed via packet capture (2026-08-07/08) across all four Accolades
...
-- values are identical everywhere - only the confirm-quantity value changes,
-- and only 1/50/99 have been observed and confirmed. No safe general formula
-- for arbitrary quantities could be derived from these three points alone,
-- so purchases are built from these three known-good chunk sizes.
local POWDER_START  = {h1 = 10,  h2 = 0};
local POWDER_SELECT = {h1 = 387, h2 = 0};
local POWDER_QTY_STEPS = {
    [99] = {h1 = 0x6184, h2 = 0x000C},
    [50] = {h1 = 0x4184, h2 = 0x0006},
    [1]  = {h1 = 0x2184, h2 = 0x0000},
};
        ...
        if (os.time() >= (buyTimer + buyDelay)) then
            ...
            if (#buyQueue == 0) then
                ...
                if purchase_item then
                    local post_qty = count_item_qty(purchase_item.Id);
                    local obtained = post_qty - pre_purchase_qty;

                    if obtained >= total_requested then
                        print(string.format("\30\110[jpmidnight] Purchase complete: obtained %d x %s.", obtained, purchase_item.Name));
                    elseif obtained > 0 then
                        print(string.format("\30\68[jpmidnight] Only obtained %d of %d requested %s (vendor stopped granting more - daily limit or other cap likely reached).", obtained, total_requested, purchase_item.Name));
                    else
                        print(string.format("\30\68[jpmidnight] Obtained 0 of %d requested %s - purchase was rejected (daily limit or other cap likely reached).", total_requested, purchase_item.Name));
                    end;
...
function start_buy_run(item, vendor_type)
    ...
    if vendor_type == "accolades" then
        ...
        while remaining > 0 do
            ...
            table.insert(buyQueue, {h1 = POWDER_QTY_STEPS[chunk].h1, h2 = POWDER_QTY_STEPS[chunk].h2});
...
function reset_me()
    ...
    if purchase_item then
        local post_qty = count_item_qty(purchase_item.Id);
        local obtained = post_qty - pre_purchase_qty;
        print(string.format("\30\110[jpmidnight] State reset and menu force closed. Obtained %d of %d requested %s before reset.", obtained, total_requested, purchase_item.Name));
        purchase_item = nil;
    else
