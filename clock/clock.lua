...
local function print_help(isError)
    ...
    local cmds = T{
        ...
        { '/time (p | pos | position) <x> <y>', 'Sets the clock screen position.' },
    };
...
ashita.events.register('unload', 'unload_cb', function ()
    ...
    if (clock.font ~= nil) then
        -- Capture the current on-screen position before destroying the font object..
        -- (Covers the case where the user shift-dragged the clock to a new position.)
...
ashita.events.register('command', 'command_cb', function (e)
    ...
    if (#args >= 2 and args[2]:any('save')) then
        -- Capture the current on-screen position first, in case the clock was shift-dragged..
        if (clock.font ~= nil) then
            clock.settings.font.position_x = clock.font.position_x;
            clock.settings.font.position_y = clock.font.position_y;
        end
    ...
    -- Handle: /time p <x> <y> - Sets the clock screen position.
    -- Handle: /time pos <x> <y> - Sets the clock screen position.
    -- Handle: /time position <x> <y> - Sets the clock screen position.
    if (#args >= 4 and args[2]:any('p', 'pos', 'position')) then
        local x = args[3]:num_or(nil);
        local y = args[4]:num_or(nil);

        if (x == nil or y == nil) then
            print(chat.header(addon.name):append(chat.error('Invalid position given.')));
            return;
        end

        -- Set the clock position..
        clock.settings.font.position_x = x;
        clock.settings.font.position_y = y;
        settings.save();
        ...
        print(chat.header(addon.name):append(chat.message('Clock position set to: '))
            :append(chat.success('%d, %d'))
            :fmt(x, y));
        return;
    end
