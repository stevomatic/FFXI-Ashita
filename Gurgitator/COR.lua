...
local sets = {
    ...
    ['dummy'] = {
        ...
        Range = { Name = 'Death Penalty', AugPath='A' },
        Ammo = 'Living Bullet',
        Head = 'Nyame Helm',
        Neck = 'Warder\'s Charm +1',
        Ear1 = 'Eabani Earring',
        ...
        Ring1 = 'Defending Ring',
        Ring2 = 'Murky Ring',
        Back = { Name = 'Camulus\'s Mantle', Augment = { [1] = 'Damage taken-5%', [2] = 'Mag. Acc.+20', [3] = 'Weapon skill damage +10%', [4] = 'Magic Damage+20', [5] = 'AGI+20', [6] = 'Magic Damage +10' } },
        Waist = 'Carrier\'s Sash',
...
local CorSaveFile = string.format('%s\\config\\addons\\luashitacast\\cor_modes.txt', AshitaCore:GetInstallPath());

local function SaveModes()
    local f = io.open(CorSaveFile, 'w');
    if (f ~= nil) then
        f:write(MeleeMode .. ',' .. RangedMode);
        f:close();
    end
end

local function LoadModes()
    local f = io.open(CorSaveFile, 'r');
    if (f == nil) then
        return;
    end
    ...
    local m, r = contents:match('(%d+),(%d+)');
    m = tonumber(m);
    r = tonumber(r);

    if (m ~= nil) and (m >= 1) and (m <= #MeleeModes) then
        MeleeMode = m;
    end
    if (r ~= nil) and (r >= 1) and (r <= #RangedModes) then
        RangedMode = r;
    end

    gFunc.Message('Restored - Main: ' .. MeleeModes[MeleeMode] .. ', Range: ' .. RangedModes[RangedMode]);
end

profile.OnLoad = function()
    gSettings.AllowAddSet = true;
    AshitaCore:GetChatManager():QueueCommand(1, '/macro book 003');
    AshitaCore:GetChatManager():QueueCommand(1, '/macro set 001');
    AshitaCore:GetChatManager():QueueCommand(1, '/lockstyleset 003');
    AshitaCore:GetChatManager():QueueCommand(1, '/bind = /lac fwd main');
    AshitaCore:GetChatManager():QueueCommand(1, '/bind - /lac fwd range');
    LoadModes();
end
